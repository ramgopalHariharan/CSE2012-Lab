...
sortItemsByRatio(Item items[], int n) {
	for(int i=1;i<n;i++) {
		Item x = items[i];
		int j = i-1;
		while(j>=0 && items[j] < x) {
...
float KnapsackGreedy3(Item items[], int n, int W) {
	...
	for(int i=0;i < n && W > 0 ;i++) {
		if(W > items[i].weight) {
			...
			float taken = (float) W / items[i].weight;
...
int main() {
	...
	int n, W;
	input(items, n, W);
	cout<<"\nGreedy Algorithm using Profit/Weight Ratio\n";
	float MaxValue = KnapsackGreedy3(items, n, W);
