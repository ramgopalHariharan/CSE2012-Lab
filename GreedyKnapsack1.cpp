...
sortItemsByValue(Item items[], int n) {
	for(int i=1;i<n;i++) {
		Item x = items[i];
		int j = i-1;
		while(j>=0 && items[j] < x) {
...
float KnapsackGreedy1(Item items[], int n, int W) {
	sortItemsByValue(items, n);
	float MaxValue = 0;
...
int main() {
	...
	int n, W;
	input(items, n, W);
	cout<<"\nGreedy Algorithm using Profit\n\n";
	float MaxValue = KnapsackGreedy1(items, n, W);
